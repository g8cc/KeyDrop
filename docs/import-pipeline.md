...
API 模式的零触碰覆盖**读路径**:endpointInfo(端口+客户端 key,供常驻入口/回写自愈/链路探测)与代理池巡检(auth-dir 账号扫描)都走管理 API,不 stat 不读文件——config.yaml 与 auth-dir 在 ~/Documents 下时,任何 stat 都可能是一次"文稿"弹窗(更新换 ad-hoc 签名使 TCC 授权作废后)。v1.4.31 曾因启动对账调 endpointInfo 使弹窗变成"启动必弹",v1.4.32/33 补齐全部读路径。彻底消除弹窗的另一腿:构建机创建 `KeyDropLocal` 签名证书(make-app.sh 指引),让 TCC 授权跨更新保持。
